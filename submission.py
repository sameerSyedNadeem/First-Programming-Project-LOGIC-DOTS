...
def GameSolver(filename):
    ...
    for i in range(len(al)):
        if checkRules(al[i], cr[1])==True and isValidBoard(al[i],cr[0]):
            return al[i]
    return []
def getRules_bonus(rules):
    n=rules.split('\n')
    new=[]
    rul=[]
    for i in range(len(n)):
        new.append(n[i].split(' '))
        new[i]=' '.join(new[i][1:])
        rul.append(new[i].replace('.', ''))
    return rul
def inputFromfiles_bonus(filename):
    f=open(filename, 'r')
    z=f.read().split('\n')
    if z[len(z)-1]=='':
        r=z[4:(len(z)-1)]
        ...
        r='\n'.join(r)
    c=z[1].replace(',','')
    return (getColors(c),getRules_bonus(r))
def checkRules_bonus(board,rules):
    for i in range(len(rules)):
        r=rules[i].split(' ')
        if r[len(r)-1]==('left' or 'right'):
            if r[0]==('2' or '3'):
                if not inColumn(board, r[len(r)-1], r[1], int(r[0])):
                    return False
            else:
                if not inColumn(board, r[len(r)-1], r[0], 1)
                    return False
        if r[len(r)-1]==('bottom' or 'top'):
            if r[0]==('2' or '3'):
                if not inColumn(board, r[len(r)-1], r[1])
getRules_bonus('1. 2 Green cubes on the left.\n2. 2 Teal cubes at the bottom.\n3. 3 LightBlue cubes are at the top.\n4. Orange cube on the right')
#print(inputFromfiles_bonus('board1.txt'))